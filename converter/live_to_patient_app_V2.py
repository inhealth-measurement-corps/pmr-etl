...
# -*- coding: utf-8 -*-
"""
changed on April 7 2021 
@author: Mung Yao
Track number of ambulations, speed, duration, distance 
"""
import os.path, time
import pymssql
import numpy as np
...
file = open("Live_Ambulation_Unit.csv", "w")
file.write("Live_Ambulation_Unit \n") 
writer = csv.writer(file)
file.close()

file2 = open("Live_Ambulation_Daily.csv", "w")
file2.write("Live_Ambulation_Daily \n") 
...
cur.execute("SELECT * FROM mmambulation.live_details") #get the ambulation table
...
for row in cur.fetchall():
    if patient_map.get(row[0]) != None: #if the patient is already in our dict 
        ...
        cur2.execute("SELECT * FROM mmambulation.patient_info WHERE patient_ID = %s", (row[0],))
        row2 = cur2.fetchone()
        if (row2 != None):
            admission_date = datetime.strptime(str(row2[3]), '%Y-%m-%d').date() 
            transfer_date = datetime.strptime(str(row2[4]), '%Y-%m-%d').date()
            #patient_ID, room_number, badgeID, admission_date, transfer_date, length_of_stay
            if (row2[2] != None): 
                patient = Patient(row[0], row2[2], row2[1], admission_date, transfer_date, None, row2[7])
                
            start_date = datetime.strptime(str(row[2]), '%Y-%m-%d').date()
            ambulation = Ambulation(str(row[0]), start_date, row[3], row[4], row[5], row[6], row[1])
            patient.add_ambulation(ambulation)
...
#with open("Live_Ambulation_Unit.csv", 'a', newline='') as csvfile:
with open("Live_Ambulation_Unit.csv", 'a') as csvfile:
...
#with open("Live_Ambulation_Daily.csv", 'a',newline='') as csvfile:
with open ("Live_Ambulation_Daily.csv", 'a') as csvfile:
    filewriter = csv.writer(csvfile, delimiter=',',
                            quotechar='|', quoting=csv.QUOTE_MINIMAL)
