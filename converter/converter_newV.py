...
cur.execute("SELECT * FROM mmambulation.historical_details") #get the ambulation table
...
for row in cur.fetchall():
    if patient_map.get(row[0]) != None: #if the patient is already in our dict 
        ...
        if (row2 != None):
            ...
            discharge_date = datetime.strptime(str(row2[6]), '%Y-%m-%d').date()
            #patient_ID, room_number, badgeID, bn, admission_date, transfer_date, discharge_date, length_of_stay
            if (row2[2] != None): 
                patient = Patient(row[0], row2[2], row2[1], admission_date, transfer_date, discharge_date, row2[7])
...
with open("Ambulation_Unit.csv", 'a') as csvfile:
...
with open ("Ambulation_Daily.csv", 'a') as csvfile:
    filewriter = csv.writer(csvfile, delimiter=',',
                            quotechar='|', quoting=csv.QUOTE_MINIMAL)
